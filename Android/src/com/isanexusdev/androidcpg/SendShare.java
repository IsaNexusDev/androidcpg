...
public class SendShare extends Activity {
	...
	private Button mShowResultButton = null;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mShowResultButton.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View view) {
				WebDialog dialog = new WebDialog(SendShare.this, mLastResult);
				...
				public void result(int result, IsLoggedInAsyncTask isLoggedInAsyncTask) {
					...
					if (result == 1) {
						...
						if (AndroidCPG.isDoubleLogin() && loginAtteps == 1){
							...
							dissmissProgress();
							Toast.makeText(SendShare.this, R.string.loginfailed, Toast.LENGTH_SHORT).show();
							notLogged();
	...
	void isLogged(){
		...
		Utils.fetchAlbums(new FetchAlbumsAsyncTask.FetchAlbumsListener() {
			...
			public void result(FetchAlbumsAsyncTask task, int result) {
				...
				if (result == 1) {
					if (task.canCreate() == false && task.getAlbums().size() == 0 || task.getCatId() == 0 || task.getCatPos() == 0){
						mLastResult = task.getReply();
					}
					populateAlbumsFromTask(task.getAlbums(), task.canCreate(), task.getCatId(), task.getCatPos());
				} else {
					Toast.makeText(SendShare.this, R.string.albumsfetchingfailed, Toast.LENGTH_SHORT).show();
					notLogged();
					mLastResult = task.getReply();
				}
